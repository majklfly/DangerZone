...
import { PDFAnimation } from "../LottieAnimations/LottieAnimations";
...
const PDFCertificate = (props) => {
  ...
  const setChapterNames = () => {
    const chaptersLocal = [];
    if (props.userData.firstName.length < 2) {
      setErrorMessage("we are missing your first name");
    } else if (props.userData.lastName < 2) {
      setErrorMessage("we are missing your last name");
    } else if (props.userData.chapterdata.length === 0) {
      setErrorMessage("you haven't finished any course yet");
    } else {
  ...
  return (
    <>
      {errorMessage && (
        <h2 className="errorMessagePDF">
          I'm sorry, you can't download this certificate because {errorMessage}
        </h2>
      )}
      <button
